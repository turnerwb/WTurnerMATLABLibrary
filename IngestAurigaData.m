...
%%Calculate h21, fT
h21 = zeros(PIV_PPT,max(PIV_TraceNum)+1,length(freqs));
...
%NB 4/10/25 Looks good now, reincluding it
for i=1:PIV_PPT
    for j=1:max(PIV_TraceNum)+1
       ...
       % pause
        best_gof = -inf;
        for idx=0:length(freqs)-window_size-1
            [fit_res, gof] =fit(log10(freqs(1+idx:1+idx+window_size)),squeeze(h21(i,j,1+idx:1+idx+window_size)),'poly1','Lower',[-20,-inf],'Upper',[-20,inf]);
            if gof.rsquare > best_gof
                best_fit = fit_res;
                best_gof = gof.rsquare;
                if best_gof > 0.99
                    break;
                end
            end
        end
        h21_Fits{i,j} = best_fit;
        %[h21_Fits{i,j}, gof] = fit(log10(freqs(2:fidx_end)),squeeze(h21(i,j,2:(fidx_end))),'poly1','Lower',[-20,-inf],'Upper',[-20,inf]);
        best_gof = gof.rsquare;
        try
        ...
        if (min(h21(i,j,:)) <= 0 && max(h21(i,j,:))<=0)
                fT(i,j) = 0;
                disp("NEVER ABOVE ZERO")
                figure(100)
                semilogx(freqs,squeeze(h21(i,j,:)))
                hold on
                plot(freqs,log10(freqs)*mh(i,j)+bh(i,j))
                hold off
                %pause
        %elseif(abs(mh(i,j) + 20) < 5)
        elseif(best_gof >0.9)
